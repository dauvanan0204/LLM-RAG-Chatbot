...
def seed_milvus(URI_link: str, collection_name: str, filename: str, directory: str, use_ollama: bool = False) -> Milvus:
    ...
    if use_ollama:
        embeddings = OllamaEmbeddings(
            model="nomic-embed-text"  # hoặc model khác mà bạn đã cài đặt
        )
    else:
        embeddings = OpenAIEmbeddings(model="text-embedding-3-large")
    
    print(f"[INFO] Using embeddings: {type(embeddings).__name__}")
...
def connect_to_milvus(URI_link: str, collection_name: str, use_ollama: bool = False) -> Milvus:
    ...
    Args:
        ...
        use_ollama (bool): Dùng mô hình embeddings từ Ollama (nếu True) hoặc OpenAI (nếu False)
    Returns:
...
def main():
    ...
    seed_milvus('http://localhost:19530', 'data_test', 'stack.json', 'data', use_ollama=True)
